...
class SequenceNode(FlowNode):
    ...
    steps: List[Union["AgentNode", "SequenceNode", "LoopNode", "IfNode", "SwitchNode"]]

class LoopNode(FlowNode):
    """循环执行，直到满足停止条件"""
    node_type: Literal["loop"] = "loop"
    body: Union["AgentNode", "SequenceNode", "LoopNode", "IfNode", "SwitchNode"]
...
class IfNode(FlowNode):
    ...
    true_body: Union["AgentNode", "SequenceNode", "LoopNode", "IfNode", "SwitchNode"]
    false_body: Optional[Union["AgentNode", "SequenceNode", "LoopNode", "IfNode", "SwitchNode"]] = None

class SwitchNode(FlowNode):
    """多路分支（用于 agent_mode 选择）"""
    node_type: Literal["switch"] = "switch"
    variable: str               # 上下文变量名，例如 "agent_mode"
    cases: Dict[str, Union["AgentNode", "SequenceNode", "LoopNode", "IfNode", "SwitchNode"]]  # 映射关系: "fibre" -> FlowNode
    default: Optional[Union["AgentNode", "SequenceNode", "LoopNode", "IfNode", "SwitchNode"]] = None

# 顶层 Flow 对象
class AgentFlow(BaseModel):
    name: str
    description: str = ""
    root: Union[AgentNode, SequenceNode, LoopNode, IfNode, SwitchNode]
