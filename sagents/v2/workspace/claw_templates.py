"""V2-owned snapshot of the desktop-v1.1.8 Claw bootstrap templates.

The template text is frozen from the V1 release tag. This module never imports
or reads a V1 runtime module, workspace, database, or user content.
"""
...
_DOCUMENTS_BY_LANGUAGE: dict[str, dict[str, str]] = {
    language: {
        "AGENT.md": default_agent_md[language],
        "IDENTITY.md": default_identity_md[language],
        "SOUL.md": default_soul_md[language],
        "USER.md": default_user_md[language],
        "MEMORY.md": default_memory_md[language],
    }
...
def _supported_language(value: str) -> str | None:
    ...
    if not normalized or normalized in {"c", "c.utf-8", "posix", "system"}:
        return None
...
def _system_language() -> str:
    for variable in ("LANGUAGE", "LC_ALL", "LC_MESSAGES", "LANG"):
        selected = _supported_language(os.environ.get(variable, ""))
    ...
    try:
        with preferences.open("rb") as stream:
            languages = plistlib.load(stream).get("AppleLanguages", ())
        if isinstance(languages, (list, tuple)):
            for value in languages:
                selected = _supported_language(str(value))
                if selected is not None:
                    return selected
...
def claw_workspace_documents(language: str) -> dict[str, str]:
    """Return a copy of the desktop-v1.1.8 Claw bootstrap documents."""

    selected = _supported_language(language)
    if selected is None:
        selected = _system_language() if language.strip().lower() == "system" else "en"
    return dict(_DOCUMENTS_BY_LANGUAGE[selected])
