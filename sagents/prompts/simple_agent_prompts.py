...
2. 继续执行任务：
  ...
  - 当最后一条 Assistant 消息中明确表示“接下来还要继续生成/整理/总结/分析/执行下一步”，即使当前看起来已经产出了一部分结果，也必须判断继续执行任务，不能过早结束。
  - 如果最后一条 Assistant 消息是在汇报“某个中间产物已完成”，但紧接着又说明还要基于该中间产物继续加工（例如再生成摘要、再整理成最终答复、再补充说明），必须判断继续执行任务。
  - 只有当 Assistant 的最后表达已经是面向用户的最终交付，且没有任何“接下来/然后/我将/现在让我/下一步”等继续执行意图时，才可以判断任务完成。

## 输出内容一致对齐逻辑
1. 如果reason 是等待工具调用，则task_interrupted是false
2. 如果最后一条 Assistant 消息包含明显的继续执行意图，则task_interrupted 必须是false
