...
class SageAsyncOpenAI:
    ...
    def __init__(
        ...
        self.model_name = model_name or getattr(
            standard_client, "model_name", "unknown"
        )
        self.fast_model_name = fast_model_name

        # 暴露 base_url 和 api_key 属性（从底层客户端获取）
        self.base_url = getattr(standard_client, "_base_url", None) or getattr(
            standard_client, "base_url", None
        )
        self.api_key = getattr(standard_client, "_api_key", None) or getattr(
            standard_client, "api_key", None
        )

        # 常见能力位直接挂到对象上，便于调用点与日志中读取。
        self.supports_multimodal = bool(
            self.model_capabilities.get("supports_multimodal", False)
        )
        self.supports_structured_output = bool(
            self.model_capabilities.get("supports_structured_output", False)
        )
