            ...
            try:
                ...
                for attr_name in dir(module):
                    if (
                        ...
                        and attr_name != "AGENT_IDENTIFIER"
                        and not attr_name.endswith("_PROMPTS_ZH")
                        and not attr_name.endswith("_PROMPTS_EN")
                        and not attr_name.endswith("_PROMPTS_PT")
                    ):
                        attr_value = getattr(module, attr_name)
                        # 检查是否是包含zh或en键的字典
                        if isinstance(attr_value, dict) and (
                            "zh" in attr_value or "en" in attr_value
                        ):
                            if "zh" in attr_value:
                ...
                if zh_prompts or en_prompts or pt_prompts:
                    ...
                    logger.debug(
                        f"成功导入prompt模块: {name} (agent: {agent_identifier}, zh: {len(zh_prompts)}, en: {len(en_prompts)}, pt: {len(pt_prompts)})"
                    )
                else:
                    # 兼容旧格式：查找模块中的PROMPTS变量
                    for attr_name in dir(module):
                        if (
                            attr_name.endswith("_PROMPTS_ZH")
                            or attr_name.endswith("_PROMPTS_EN")
                            or attr_name.endswith("_PROMPTS_PT")
                        ):
                    ...
                    logger.debug(
                        f"成功导入prompt模块(旧格式): {name} (agent: {agent_identifier})"
                    )
...
class PromptManager:
    ...
    def _load_agent_prompts(self):
        ...
        print(
            f"Agent prompt加载完成: 中文{len(self.agent_prompts_zh)}个, 英文{len(self.agent_prompts_en)}个, 葡萄牙语{len(self.agent_prompts_pt)}个"
        )
    ...
    def get_prompt(
        ...
        if agent:
            ...
            agent_prompts = (
                self.agent_prompts_map.get(language) or self.agent_prompts_en
            )
    ...
    def format(
        self, key: str, agent: Optional[str] = None, language: str = "en", **kwargs
    ) -> str:
        """格式化prompt模板
    ...
    def get_agent_prompt(
        self, agent: str, key: str, language: str = "en", default: Optional[str] = None
    ) -> str:
        """获取指定agent的prompt
    ...
    def get_agent_prompt_auto(
        self, key: str, language: str = "en", default: Optional[str] = None
    ) -> str:
