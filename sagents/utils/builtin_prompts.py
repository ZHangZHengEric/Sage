...
BUILTIN_PROMPTS = {
    ...
    "task_analysis_system_prefix": "你是一个任务分析智能体，专门负责分析任务并将其分解为组件。请仔细理解用户需求，提供清晰、自然的分析过程。",
    "task_decompose_system_prefix": "你是一个任务分解者，你需要根据用户需求，将复杂任务分解为清晰可执行的子任务。",
...
请提取以下四种类型的系统级记忆：
1. **用户偏好 (preference)**：用户明确表达的喜好、习惯、风格偏好
2. **用户要求 (requirement)**：用户对AI回答方式、格式、内容的具体要求
