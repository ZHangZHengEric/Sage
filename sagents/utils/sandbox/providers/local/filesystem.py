...
class SandboxFileSystem:
    ...
    def to_host_path(self, virtual_path: str) -> str:
        ...
        for mapped_virtual, mapped_host in self._iter_virtual_mappings():
            ...
            if normalized_path.startswith(
                mapped_virtual + os.sep
            ) or normalized_path.startswith(mapped_virtual + "/"):
                rel_path = (
                    normalized_path[len(mapped_virtual) :].lstrip(os.sep).lstrip("/")
                )
    ...
    def to_virtual_path(self, host_path: str) -> str:
        ...
        for mapped_host, mapped_virtual in self._iter_host_mappings():
            ...
            if normalized_host.startswith(
                mapped_host + os.sep
            ) or normalized_host.startswith(mapped_host + "/"):
                rel_path = (
    ...
    def map_text_to_host(self, text: str) -> str:
        ...
        for virtual_path, host_path in self._iter_virtual_mappings():
            ...
            pattern = (
                r"(?<![a-zA-Z0-9_\.\-/])" + escaped_path + r"(?=$|/|[^a-zA-Z0-9_\.\-])"
            )
            result = re.sub(
                pattern, lambda m, replacement=host_path: replacement, result
            )
        return result
    ...
    def map_text_to_virtual(self, text: str) -> str:
        ...
        for host_path, virtual_path in self._iter_host_mappings():
            ...
            result = re.sub(
                pattern, lambda m, replacement=virtual_path: replacement, result
            )
        return result
    ...
    async def write_file(
        ...
        return await asyncio.to_thread(
            self._write_file_sync, path, content, encoding, append
        )
