"""
测试 LocalSandboxProvider 的超时返回 stdio 功能
...
class TestExecuteCommandTimeout:
    ...
    @pytest.fixture
    async def sandbox(self, tmp_path):
        """创建临时沙箱"""
    ...
    async def test_timeout_with_stderr_output(self, tmp_path):
        ...
        try:
            ...
            # stderr 被合并到 stdout 了
            assert "STDOUT_MSG" in result.stdout
            # 注意：在 local.py 中 stderr 被合并到 stdout 了
