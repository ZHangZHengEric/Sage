...
class BoundLogger:
    def __init__(
        self, base_logger: "Logger", context: Optional[Dict[str, object]] = None
    ):
...
class Logger:
    ...
    def __init__(self, log_dir=None):
        ...
        console_format = logging.Formatter(
            "%(asctime)s - %(levelname)s - [%(session_id)s] - [%(caller_filename)s:%(caller_lineno)d] - %(message)s"
        )
    ...
    def _cleanup_old_logs(self):
        ...
        try:
            ...
            log_patterns = [
                ...
                os.path.join(
                    self.log_dir, "sage_[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9].log"
                ),  # 旧格式日期文件
            ]
            ...
            for log_file in all_log_files:
                try:
                    ...
                    if file_mtime < one_month_ago:
                        ...
                        print(
                            f"[LOG CLEANUP] Deleted old log file: {log_file} (size: {file_size} bytes, modified: {file_mtime.strftime('%Y-%m-%d %H:%M:%S')})"
                        )
    ...
    def _start_periodic_cleanup(self):
        ...
        try:
            ...
            self._cleanup_timer = threading.Timer(
                self._cleanup_interval, self._periodic_cleanup_task
            )
    ...
    def _periodic_cleanup_task(self):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"[LOG CLEANUP] Failed to restart periodic cleanup: {restart_error}"
                )
    ...
    def _get_session_logger(self, session_id: str) -> logging.Logger:
        ...
        if not has_file_handler:
            try:
                ...
                session = (
                    session_manager.get_live_session(session_id)
                    if session_manager
                    else None
                )
    ...
    def _log(self, level, message, explicit_session_id: Optional[str] = None, **kwargs):
        ...
        session_id = (
            explicit_session_id or self._get_current_session_id() or "NO_SESSION"
        )
