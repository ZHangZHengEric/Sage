...
| **sage-jaeger** | 16686/4317/4318 | **4317/4318** | Jaeger 2.16 追踪采集与查询服务 |
...
> Jaeger Dashboard 现在直接挂在 `sage-web` 域名下的 `/jaeger/`。`sage-web` 会先回 Sage 校验当前会话对应用户是否仍然是 `admin`，未登录时再跳转 Sage 配置的 OIDC/OAuth 登录流程，登录成功后回到 Jaeger Dashboard。
...
| `--workspace` | `SAGE_SESSION_DIR` | `sessions` | 会话目录（存放会话记录等） |
...
| 命令行参数 | 环境变量 | 说明 |
| :--- | :--- | :--- |
| `--trace_jaeger_url` | `SAGE_TRACE_JAEGER_URL` | Jaeger OTLP gRPC 端点 (例如: `http://localhost:4317`) |
| - | `SAGE_TRACE_JAEGER_PUBLIC_URL` | Jaeger Dashboard 对外访问地址 (例如: `http://localhost:30051/jaeger`) |
| - | `SAGE_TRACE_JAEGER_UI_URL` | Jaeger Query/UI 内部地址 (例如: `http://sage-jaeger:16686/jaeger`) |
| `--enable_trace` | `SAGE_ENABLE_TRACE` | 是否开启链路追踪 (默认: True) |

在 `docker-compose.yml` 中，观测链路会按如下方式工作：

1. `sage-server` 通过 OTLP 将 traces 上报到 `sage-jaeger:4317`
2. `sage-jaeger` 使用 Jaeger `v2.16` 提供 Query/UI 能力
3. `sage-web` 通过 `/jaeger/` 对外暴露 Dashboard，并在每次访问时向 Sage 回查当前用户是否为管理员
4. 如果未登录，`sage-web` 会将浏览器跳转到 Sage 配置的 OIDC/OAuth 登录流程，登录成功后返回 Jaeger Dashboard

如果部署在远程主机上，请将 `SAGE_TRACE_JAEGER_PUBLIC_URL` 改成真实可访问的域名或地址，而不是默认的 `http://127.0.0.1:30051/jaeger`。
