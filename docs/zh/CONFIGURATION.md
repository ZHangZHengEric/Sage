...
- `SAGE_AUTH_MODE=trusted_proxy`
  使用受信任代理接入模式。Sage 仍保留本地用户名密码登录，但只允许管理员登录；如果请求来自受信任代理并透传了 `X-Sage-Internal-UserId`，该用户会默认以 `admin` 角色写入请求上下文。因此这一模式要求业务侧确保上游代理的安全性和可信度。
...
`SAGE_TRUSTED_IDENTITY_PROXY_IPS` 支持逗号分隔的 IP 或 CIDR 白名单。它的作用是校验请求来源是否可以被视为受信任身份代理。只有来源命中该白名单时，Sage 才会接受可选的 `X-Sage-Internal-UserId` 并写入请求上下文：在 `trusted_proxy` 模式下默认角色是 `admin`，其他模式下默认角色是 `user`。
