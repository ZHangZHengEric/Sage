    ...
    <div v-show="isExpanded" class="preview-content">
      ...
      <div v-else-if="fileType === 'pdf'" class="pdf-preview">
        <div class="p-4 text-center text-muted-foreground bg-muted/30">
          ...
          <p class="text-xs text-muted-foreground/60 mb-3">由于浏览器安全限制，无法直接预览本地 PDF</p>
          <Button 
            variant="outline" 
            size="sm"
            @click="openFile"
...
const previewHeight = computed(() => {
  return fileType.value === 'pdf' ? 500 : 400
})
...
const toggleExpand = () => {
  ...
  if (isExpanded.value && !fileContent.value && !loading.value && props.autoLoad) {
    loadContent()
  }
}
