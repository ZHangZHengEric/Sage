...
########################################
# Sage Desktop GitHub CI Build Script
########################################
...
build_python_sidecar() {
    ...
    local NODE_CACHE_DIR="$CACHE_DIR/node-runtime/${OS_TYPE}-${ARCH}"
    local SETUP_SCRIPT="$APP_DIR/scripts/setup-node-runtime.sh"
    
    if [ -d "$NODE_CACHE_DIR" ] && { [ -f "$NODE_CACHE_DIR/bin/node" ] || [ -f "$NODE_CACHE_DIR/node.exe" ]; }; then
        echo "[Node.js] 使用缓存的 Node.js 运行时: $NODE_CACHE_DIR"
        rm -rf "$NODE_DIR"
        mkdir -p "$NODE_DIR"
        cp -a "$NODE_CACHE_DIR/." "$NODE_DIR/"
        ...
        echo "[Node.js] 缓存 Node.js 运行时到: $NODE_CACHE_DIR"
