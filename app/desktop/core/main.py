            ...
            try:
                ...
                if pid_data:
                    ...
                    try:
                        ...
                        _safe_print(f"Warning: Another Sage Desktop instance is running (PID: {old_pid}), terminating it...", flush=True)
                        try:
                        ...
                        _safe_print(f"Warning: Stale PID file found (PID {old_pid} not running), removing...", flush=True)
                        fcntl.flock(fd, fcntl.LOCK_UN)
                        os.close(fd)
                        pid_file.unlink(missing_ok=True)
                        # Retry
    ...
    try:
        ...
        if port_env:
            try:
                ...
                _safe_print(f"Invalid SAGE_PORT environment variable: {port_env}, finding free port...", flush=True)
                port = None
        else:
            port = None
