...
class OAuth2Client(Base, OAuth2ClientMixin):
    ...
    extra_config: Mapped[Dict[str, Any]] = mapped_column(JSON, nullable=False, default={})
...
class OAuth2Token(Base, OAuth2TokenMixin):
    ...
    user_id: Mapped[str] = mapped_column(String(128), nullable=False, index=True)
    grant_type: Mapped[str] = mapped_column(String(64), nullable=False, default="authorization_code")
    token_metadata: Mapped[Dict[str, Any]] = mapped_column(JSON, nullable=False, default={})
    created_at: Mapped[datetime] = mapped_column(nullable=False)
    updated_at: Mapped[datetime] = mapped_column(nullable=False)
...
class OAuth2ClientDao(BaseDao):
    ...
    async def get_all_clients(self) -> list[OAuth2Client]:
        return await BaseDao.get_all(self, OAuth2Client, order_by=OAuth2Client.client_id.asc())
...
class OAuth2AuthorizationCodeDao(BaseDao):
    ...
    async def delete(self, authorization_code: OAuth2AuthorizationCode) -> bool:
        return await BaseDao.delete_by_id(self, OAuth2AuthorizationCode, authorization_code.code_id)
