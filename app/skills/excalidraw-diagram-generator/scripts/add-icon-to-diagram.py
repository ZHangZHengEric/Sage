...
def transform_icon_elements(
    elements: List[Dict[str, Any]], target_x: float, target_y: float
) -> List[Dict[str, Any]]:
    """
...
def add_icon_to_diagram(
    ...
    print(
        f"  Added {len(transformed_elements)} elements (total: {original_count} -> {len(diagram['elements'])})"
    )
...
def main():
    ...
    if len(sys.argv) < 5:
        print(
            "Usage: python add-icon-to-diagram.py <diagram_path> <icon_name> <x> <y> [OPTIONS]"
        )
        print("\nOptions:")
