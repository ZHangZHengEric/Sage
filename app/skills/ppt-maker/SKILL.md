...
9.  **每个PPT页面单独一个文件**：不要把所有的xml 都放到一个文件中。
...
**每个页面单独一个xml文件** 
1.  **对照大纲**：根据大纲文档中的每页规划，选择合适的布局模板
2.  **参考结构**：不要机械地替换占位符。仔细观察模板中的 **布局比例**、**装饰元素**（如 `ppt-rect` 的线条、背景色块）以及 **留白处理**
3.  **编写代码**：参考模板的 XML 结构，编写符合大纲内容需求的页面代码
4.  **灵活调整**：
    *   如果内容较少，增加装饰性卡片或放大图表
    *   如果内容较多，拆分为多页或使用更紧凑的列表布局
    *   **必须保留模板中的美学特征**（如配色、圆角、线条装饰），这是保证"好看"的关键
