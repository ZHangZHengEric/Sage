...
from app.server_v2.api.deps import CredentialDep, CurrentUser
...
async def list_keys(user: CurrentUser, credentials: CredentialDep):
    keys = await credentials.list_for(user.user_id)
    return success([item.public_dict() for item in keys])


@router.post("/api/keys", response_model=ApiResponse[ApiKeyCreated])
async def create_key(body: ApiKeyBody, user: CurrentUser, credentials: CredentialDep):
    record, token = await credentials.create(
...
async def revoke_key(key_id: str, user: CurrentUser, credentials: CredentialDep):
    record = await credentials.revoke(key_id, user.user_id)
