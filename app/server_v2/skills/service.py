...
from app.server_v2.catalog.records import AgentRecord, UserCatalog
...
from app.server_v2.skills.repository import SkillStore
...
class SkillCatalogService:
    ...
    async def list_visible(self, *, user_id: str, role: str):
        return await self.store.list_visible(user_id=user_id, role=role)
    ...
    async def get(self, skill_id: str, *, user_id: str, role: str) -> SkillRecord:
        ...
        if record is None or not _can_see(record, user_id=user_id, role=role):
            raise SageV2Error(
                RuntimeErrorInfo(
                    code="server.skills.service.not_found",
    ...
    async def publish_markdown(
        ...
        *,
        ...
        if dimension == "system" and role != "admin":
            raise SageV2Error(
                RuntimeErrorInfo(
                    code="server.skills.service.forbidden",
                    category=ErrorCategory.AUTHORIZATION,
                    message="only admin can publish system skills",
                )
            )
        ...
        for filename, payload in items:
            try:
                if not str(filename).lower().endswith(".zip"):
                    raise SageV2Error(
                        RuntimeErrorInfo(
                            code="server.skills.service.validation",
                            category=ErrorCategory.VALIDATION,
                            message="仅支持 ZIP 文件",
                        )
                    )
        ...
        if current.dimension == "system" and role != "admin":
            raise SageV2Error(
                RuntimeErrorInfo(
                    code="server.skills.service.forbidden",
                    category=ErrorCategory.AUTHORIZATION,
                    message="only admin can edit system skills",
                )
            )
        if (
            current.dimension == "user"
        ...
        if package.name != current.name:
            raise SageV2Error(
                RuntimeErrorInfo(
                    code="server.skills.service.validation",
                    category=ErrorCategory.VALIDATION,
                    message="skill name cannot be changed",
                )
            )
        ...
        if current.dimension == "system" and role != "admin":
            raise SageV2Error(
                RuntimeErrorInfo(
                    code="server.skills.service.forbidden",
                    category=ErrorCategory.AUTHORIZATION,
                    message="only admin can delete system skills",
                )
            )
        if (
            current.dimension == "user"
        ...
        if not skill_md.is_file():
            raise SageV2Error(
                RuntimeErrorInfo(
                    code="server.skills.service.not_found",
                    category=ErrorCategory.VALIDATION,
                    message="skill artifact is missing",
                )
            )
            ...
            if source is None:
                raise SageV2Error(
                    RuntimeErrorInfo(
                        code="server.skills.service.not_found",
                        category=ErrorCategory.VALIDATION,
                        message=f"skill {skill_name!r} is not bound",
                    )
                )
    ...
    async def _publish_package(
        ...
        *,
        ...
        record = SkillRecord(
            skill_id=skill_id
            or (existing.skill_id if existing is not None else new_skill_id()),
            version_id=version_id,
