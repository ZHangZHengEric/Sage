...
def normalize_skill_name(name: str) -> str:
    ...
    if not SKILL_NAME.fullmatch(value):
        raise SageV2Error(
            RuntimeErrorInfo(
                code="server.skills.records.validation",
...
def normalize_skill_names(names: list[str] | tuple[str, ...] | None) -> list[str]:
    return list(
        dict.fromkeys(
            normalize_skill_name(name) for name in names or [] if str(name).strip()
        )
    )
    ...
    if not version or "/" in version or "\\" in version or version in {".", ".."}:
        raise SageV2Error(
            RuntimeErrorInfo(
                code="server.skills.records.validation",
                category=ErrorCategory.VALIDATION,
                message="invalid skill version id",
            )
        )
    if dimension == "system":
    ...
    if not owner or "/" in owner or "\\" in owner:
        raise SageV2Error(
            RuntimeErrorInfo(
                code="server.skills.records.validation",
                category=ErrorCategory.VALIDATION,
                message="user skills require owner_user_id",
            )
        )
    ...
    if _UNSAFE_RELATIVE.search(value) or value.startswith("../"):
        raise SageV2Error(
            RuntimeErrorInfo(
                code="server.skills.records.validation",
                category=ErrorCategory.VALIDATION,
                message="artifact path escapes the skill root",
            )
        )
    return value
    ...
    try:
        ...
        raise SageV2Error(
            RuntimeErrorInfo(
                code="server.skills.records.validation",
                category=ErrorCategory.VALIDATION,
                message="workspace skill path is invalid",
            )
        ) from exc
    return target
