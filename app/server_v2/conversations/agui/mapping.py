...
def validate_agui_id(value: str, *, field: str) -> str:
    ...
    if candidate in {".", ".."} or "/" in candidate or "\\" in candidate:
        raise SageV2Error(
            RuntimeErrorInfo(
                code="server.conversations.agui.mapping.validation",
    ...
    if not isinstance(content, list):
        raise SageV2Error(
            RuntimeErrorInfo(
                code="server.conversations.agui.mapping.validation",
                category=ErrorCategory.VALIDATION,
                message="user message content is invalid",
            )
        )
    ...
    for part in content:
        ...
        if part_type == "text":
            ...
            if (
                ...
                blocks.append(ImageBlock(uri=str(source["value"]), mime_type="image/*"))
            elif value.get("url"):
...
def latest_user_input(request: RunAgentInput) -> InputItem:
    ...
    return InputItem(
        role="user", content=_content_blocks(getattr(users[-1], "content", ""))
    )
