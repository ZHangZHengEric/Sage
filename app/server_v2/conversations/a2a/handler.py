...
from app.server_v2.conversations.a2a.context import key_from
...
class _translated:
    ...
    def __exit__(self, exc_type, exc, traceback) -> bool:
        if not isinstance(exc, SageV2Error):
            return False
        if exc.info.code.endswith("not_found"):
            factory: type[A2AError] = TaskNotFoundError
        elif exc.info.category == ErrorCategory.CONFLICT:
            factory = TaskNotCancelableError
        elif exc.info.category in {
            ...
            factory = InvalidParamsError
        raise factory(message=exc.info.message) from exc
