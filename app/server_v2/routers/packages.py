...
from app.server_v2.routers.deps import (
    AdminUser,
    PackageContextDep,
    PackageDep,
    PackageQueriesDep,
...
async def result(operation):
    try:
        ...
        raise SageV2Error(
            RuntimeErrorInfo(
                code="server.routers.packages.forbidden",
        ...
        raise SageV2Error(
            RuntimeErrorInfo(
                code=f"server.routers.packages.{reason}",
                category=category,
                message=str(exc),
            )
        ) from exc
        ...
        raise SageV2Error(
            RuntimeErrorInfo(
                code="server.routers.packages.validation",
                category=ErrorCategory.VALIDATION,
                message=f"plugin syntax error at line {exc.lineno}: {exc.msg}",
            )
        ) from exc
