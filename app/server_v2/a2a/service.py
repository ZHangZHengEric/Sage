...
from app.server_v2.a2a.card import agent_card
from app.server_v2.a2a.mapping import context_id, to_start_run
from app.server_v2.a2a.stream import task_stream
from app.server_v2.a2a.task import TaskReducer
...
class A2AService:
    ...
    async def cancel_task(self, task_id: str, key: ApiKeyRecord) -> Task:
        ...
        try:
            await self._service.cancel_run(
    ...
    async def _start(self, message: Message, key: ApiKeyRecord) -> _Started:
        ...
        try:
            run_id = await service.start_detached_run(
                "a2a",
                command,
                context,
                user_id=user_id,
    ...
    async def _resume(self, message: Message, key: ApiKeyRecord) -> _Started:
        ...
        try:
            await service.resume_detached_run(
                task_id,
                context,
                user_id=key.owner_user_id,
