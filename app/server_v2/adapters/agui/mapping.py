...
def validate_agui_id(value: str, *, field: str) -> str:
    ...
    if candidate in {".", ".."} or "/" in candidate or "\\" in candidate:
        raise ServerError("validation", f"{field} is invalid")
    try:
        return _ID.validate_python(candidate)
    except Exception as exc:
        raise ServerError("validation", f"{field} is invalid") from exc
...
def _forwarded_props(request: RunAgentInput) -> Mapping[str, Any]:
    ...
    if not isinstance(props, Mapping):
        raise ServerError("validation", "forwardedProps must be an object")
    return props
...
def _content_blocks(content: Any) -> tuple[TextBlock | ImageBlock, ...]:
    if isinstance(content, str):
        text = content.strip()
        if not text:
            raise ServerError("validation", "user message is empty")
        return (TextBlock(text=text),)
    if not isinstance(content, list):
        raise ServerError("validation", "user message content is invalid")
...
def latest_user_input(request: RunAgentInput) -> InputItem:
    ...
    if not users:
        raise ServerError("validation", "at least one user message is required")
