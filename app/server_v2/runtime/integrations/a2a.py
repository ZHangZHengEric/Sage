...
from app.server_v2.catalog.records import A2AAgentRecord
...
def to_a2a_config(record: A2AAgentRecord, *, required: bool = False) -> A2AAgentConfig:
    try:
        ...
        raise SageV2Error(
            RuntimeErrorInfo(
                code="server.runtime.integrations.a2a.validation",
                category=ErrorCategory.VALIDATION,
                message=f"invalid a2a agent {record.name}: {exc}",
            )
        ) from exc
    ...
    if config.name in errors:
        raise SageV2Error(
            RuntimeErrorInfo(
                code="server.runtime.integrations.a2a.validation",
                category=ErrorCategory.VALIDATION,
                message=errors[config.name].message,
            )
        )
