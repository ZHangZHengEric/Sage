...
- `--runtime v2` and `/runtime set v2` drive the experimental SAgents v2 backend (`sage v2 chat --json`) instead of `sage chat --json`. On v2, tool approvals are answered with `/approve`, `/remember` (approve and remember for the session) and `/deny`; questions from the runtime are answered by typing in the composer, and text typed while a run is active steers the run at its next model step (`v2_steer`; answer a pending approval first). Session ids are assigned by the v2 runtime, and the session picker still lists v1 sessions only.
