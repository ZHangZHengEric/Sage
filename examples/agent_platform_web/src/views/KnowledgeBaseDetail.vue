<template>
  <div class="knowledge-base-detail">
    ...
    <div class="tabs">
      <button :class="['tab', { active: activeTab === 'documents' }]" @click="activeTab = 'documents'">{{ t('knowledgeBase.documents') }}</button>
  ...
  <div class="tab-content" v-if="activeTab === 'documents'">
      <div class="doc-filter">
        ...
        <input ref="fileInputRef" type="file" multiple style="display:none" @change="onFileChange" accept=".doc,.docx,.pdf,.txt,.json,.eml,.ppt,.pptx,.xlsx,.xls,.csv,.md" />
  ...
  <div class="tab-content" v-else-if="activeTab === 'recall'">
    ...
    <div class="tab-content" v-else>
      <form class="settings-form" @submit.prevent="saveSettings">
        ...
        <div class="actions">
          <button type="submit" class="btn-primary" :disabled="saving">{{ saving ? t('common.save') : t('common.save') }}</button>
          <button type="button" class="btn-danger" :disabled="saving" @click="confirmDelete">{{ t('common.delete') }}</button>
        </div>
      </form>
    </div>
  </div>
...
<style scoped>
.knowledge-base-detail { padding: 24px; }
.header { background: white; border-radius: 12px; padding: 16px; margin-bottom: 16px; display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; }
.header-left { display: flex; flex-direction: column; gap: 6px; }
.header-right { display: flex; align-items: flex-start; }
.title { margin: 0; font-size: 20px; font-weight: 600; }
.intro { margin: 8px 0 0 0; color: rgba(0,0,0,0.7); }
.btn-link { background: none; border: none; color: #667eea; cursor: pointer; margin-bottom: 8px; }
.tabs { display: flex; gap: 8px; margin-bottom: 12px; }
.tab { padding: 8px 12px; border: 1px solid rgba(0,0,0,0.1); background: white; border-radius: 8px; cursor: pointer; }
.tab.active { background: #667eea; color: white; border-color: #667eea; }
.tab-content { background: white; border-radius: 12px; padding: 16px; }
.placeholder { color: rgba(0,0,0,0.6); }
.settings-form { display: flex; flex-direction: column; gap: 12px; }
.form-group { display: flex; flex-direction: column; gap: 6px; }
.form-label { font-size: 14px; font-weight: 600; }
.form-input, .form-textarea { padding: 10px 12px; border: 1px solid rgba(0,0,0,0.2); border-radius: 8px; }
.actions { display: flex; gap: 8px; }
.btn-primary { background: #667eea; color: white; border: none; border-radius: 8px; padding: 8px 12px; cursor: pointer; }
.btn-danger { background: #ef4444; color: white; border: none; border-radius: 8px; padding: 8px 12px; cursor: pointer; }
.doc-ops { display: grid; grid-template-columns: 1fr auto; gap: 8px; align-items: start; margin-bottom: 12px; }
.doc-filter { display: flex; gap: 8px; margin-bottom: 12px; }
.doc-list { overflow: auto; }
.doc-table { width: 100%; border-collapse: collapse; }
.doc-table th, .doc-table td { border-bottom: 1px solid rgba(0,0,0,0.1); padding: 8px; text-align: left; }
.action-cell { display: flex; align-items: center; gap: 8px; }
.icon-btn { display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border: none; border-radius: 6px; background: rgba(102, 126, 234, 0.1); color: #667eea; cursor: pointer; transition: all 0.2s ease; }
.icon-btn:hover:not(:disabled) { background: rgba(102, 126, 234, 0.2); transform: scale(1.05); }
.icon-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.icon-btn.danger { background: rgba(239, 68, 68, 0.1); color: #ef4444; }
.icon-btn.danger:hover:not(:disabled) { background: rgba(239, 68, 68, 0.2); }
.icon-btn.primary { background: rgba(102, 126, 234, 0.1); color: #667eea; }
.pager { display: flex; gap: 8px; align-items: center; margin-top: 12px; justify-content: center; }
.btn { border: 1px solid rgba(0,0,0,0.2); background: white; border-radius: 6px; padding: 6px 10px; cursor: pointer; }
