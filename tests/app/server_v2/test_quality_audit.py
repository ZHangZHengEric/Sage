...
@pytest.mark.asyncio
async def test_protocol_error_is_not_persisted_as_success(tmp_path, monkeypatch):
    service = make_test_service(tmp_path)
    await service.start()
    run = (
        await service.replay.claim(user_id="user", thread_id="thread", run_id="run")
    ).run
    detached = []
    ...
    async def results():
        yield SimpleNamespace(
            frames=(
                SimpleNamespace(name="RUN_ERROR", payload={"message": "model failed"}),
            )
        )
    ...
    async def run_interface(*args, **kwargs):
        return SimpleNamespace(results=results(), detach=detach)

    monkeypatch.setattr(service.application, "run_interface", run_interface)
    try:
        await service._drive_correlated_agui_run(
            run,
            _command(),
            user_id="user",
            agent_id="main",
        ...
        assert [event.payload["type"] for event in run.events] == ["RUN_ERROR"]
