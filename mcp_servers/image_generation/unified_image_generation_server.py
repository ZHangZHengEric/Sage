...
class ImageGenerationManager:
    ...
    def __init__(self):
        self.available_providers: List[BaseImageProvider] = []
    ...
    def get_reference_image_providers(self) -> List[BaseImageProvider]:
        """获取支持参考图的提供商列表"""
        return [p for p in self.available_providers if p.supports_reference_image]
    ...
    async def generate_image(
        ...
        """
        if not self.available_providers:
            return None, self.get_config_error()
        
        # 如果提供了参考图，优先使用支持参考图的提供商
        providers = self.available_providers
        if reference_image:
            ref_providers = self.get_reference_image_providers()
            if ref_providers:
        ...
        for provider in providers:
            try:
                ...
                if output_path and result.is_base64:
                    self._save_base64_image(result.image_data, output_path)
...
async def generate_image(
    ...
    """
    # 检查是否有可用的提供商
    if not image_manager.available_providers:
        config_error = image_manager.get_config_error()
        return json.dumps({
            "success": False,
    ...
    # 执行图片生成
    result, error = await image_manager.generate_image(
        prompt=prompt,
        aspect_ratio=aspect_ratio,
