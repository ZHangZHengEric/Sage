...
from .agent_config import get_agent_im_config, list_all_agents
...
class IMServiceManager:
    ...
    async def _auto_start_channels(self):
        ...
        try:
            ...
            logger.info(
                f"[ServiceManager] Found {len(configs)} legacy configs for user {DEFAULT_SAGE_USER_ID}"
            )
            ...
            for config in configs:
                ...
                logger.info(
                    f"[ServiceManager] Legacy config: provider={provider}, enabled={enabled}"
                )

                if enabled and provider:
                    logger.info(
                        f"[ServiceManager] Auto-starting legacy {provider} channel..."
                    )
                    try:
                        await self.start_channel(DEFAULT_SAGE_USER_ID, provider)
                    except Exception as e:
                        logger.error(
                            f"[ServiceManager] Failed to auto-start legacy {provider}: {e}"
                        )
            ...
            try:
                ...
                logger.info(
                    f"[ServiceManager] Found {len(agents)} agents with IM config: {agents}"
                )
                ...
                for agent_id in agents:
                    try:
                        ...
                        logger.info(
                            f"[ServiceManager] Agent {agent_id} channels: {list(all_channels.keys())}"
                        )

                        for provider, channel_data in all_channels.items():
                            logger.info(
                                f"[ServiceManager] Checking {provider} for {agent_id}: enabled={channel_data.get('enabled')}"
                            )
                            if channel_data.get("enabled"):
                                logger.info(
                                    f"[ServiceManager] Auto-starting {provider} for agent={agent_id}"
                                )
                                try:
                                    await self.start_channel(agent_id, provider)
                                except Exception as e:
                                    logger.error(
                                        f"[ServiceManager] Failed to auto-start {provider} for {agent_id}: {e}"
                                    )
                    except Exception as e:
                        logger.warning(
                            f"[ServiceManager] Failed to process agent {agent_id}: {e}",
                            exc_info=True,
                        )

            except Exception as e:
                logger.warning(
                    f"[ServiceManager] Failed to auto-start Agent configs: {e}",
                    exc_info=True,
                )
    ...
    async def start_channel(self, sage_user_id: str, provider_type: str) -> bool:
        ...
        with self._lock:
            ...
            if key in self._connections:
                ...
                if state.status in [ChannelStatus.CONNECTED, ChannelStatus.CONNECTING]:
                    logger.info(
                        f"[ServiceManager] Channel {key} already running (status={state.status.value})"
                    )
                    return True
                elif state.status == ChannelStatus.ERROR:
                    # If previous attempt failed, remove it and try again
                    logger.warning(
                        f"[ServiceManager] Channel {key} was in ERROR state, will retry"
                    )
            ...
            try:
                ...
                if channel_info:
                    ...
                    logger.info(
                        f"[ServiceManager] Using Agent-level config for {key}, enabled={config_data['enabled']}"
                    )
            except Exception as e:
                logger.info(
                    f"[ServiceManager] Failed to get Agent config for {key}: {e}",
                    exc_info=True,
                )
        ...
        try:
            ...
            if provider_type == "feishu":
                ...
                task = asyncio.create_task(
                    self._run_wechat_personal_channel(
                        sage_user_id, config_data["config"]
                    )
                )
            else:
            ...
            if state.status == ChannelStatus.ERROR:
                logger.error(
                    f"[ServiceManager] Channel {key} failed to start: {state.error_message}"
                )
                return False
        ...
        with self._lock:
            for key, state in self._connections.items():
                if (
                    ...
                    and state.status == ChannelStatus.CONNECTED
                    and state.provider
                ):
                    # 找到匹配的 provider
                    logger.debug(
                        f"[ServiceManager] Found provider for {provider_type}:{user_id}"
                    )
                    return state.provider
    ...
    async def restart_channel(self, sage_user_id: str, provider_type: str) -> bool:
        """Restart a channel."""
        logger.info(
            f"[ServiceManager] Restarting channel {sage_user_id}:{provider_type}..."
        )
    ...
    async def update_channel_config(
        self, sage_user_id: str, provider_type: str, config: Dict[str, Any]
    ) -> bool:
        """
    ...
    def get_channel_status(
        self, sage_user_id: str, provider_type: str
                    ...
                    for key, state in list(self._connections.items()):
                        ...
                        if state.task and state.task.done():
                            ...
                            if state.status != ChannelStatus.STOPPING:
                                logger.error(
                                    f"[ServiceManager] Channel {key} exited unexpectedly"
                                )
                                state.status = ChannelStatus.ERROR
                                ...
                                asyncio.create_task(
                                    self._auto_retry(
                                        state.sage_user_id, state.provider_type
                                    )
                                )
    ...
    async def _auto_retry(self, sage_user_id: str, provider_type: str, delay: int = 60):
        ...
        logger.info(
            f"[ServiceManager] Auto-retry {sage_user_id}:{provider_type} in {delay}s..."
        )
            ...
            try:
                ...
                if msg_type == "file":
                    ...
                    if file_key and message_id:
                        logger.info(
                            f"[ServiceManager] Feishu file message detected: {file_name}, file_key={file_key}"
                        )
                        ...
                        logger.info(
                            f"[ServiceManager] Downloading Feishu file: {file_name}"
                        )
                        ...
                        if download_result.get("success"):
                            ...
                            logger.info(
                                f"[ServiceManager] Feishu file downloaded: {local_path}, size={file_size}"
                            )
                            ...
                            if mime_type and (
                                ...
                                try:
                                    ...
                                    if file_content:
                                        ...
                                        logger.info(
                                            f"[ServiceManager] Added text file content, length={len(file_content)}"
                                        )
                                except Exception as e:
                                    logger.warning(
                                        f"[ServiceManager] Failed to read text file: {e}"
                                    )
                            else:
                            ...
                            logger.error(
                                f"[ServiceManager] Failed to download Feishu file: {error}"
                            )
                            message["file_info"] = {
                                "name": file_name,
                                "size": 0,
                ...
                logger.error(
                    f"[ServiceManager] Error in Feishu message handler: {e}",
                    exc_info=True,
                )
        ...
        async def handler(message: Dict[str, Any]):
            ...
            try:
                ...
                if file_info and file_info.get("download_code"):
                    ...
                    logger.info(
                        f"[ServiceManager] DingTalk file message detected, download_code: {download_code}"
                    )
                    ...
                    logger.info(
                        f"[ServiceManager] Downloading DingTalk file: {file_name}, code={download_code[:20]}..."
                    )
                    ...
                    try:
                        ...
                        if download_result.get("success"):
                            ...
                            logger.info(
                                f"[ServiceManager] DingTalk file downloaded: {local_path}, size={file_size}"
                            )
                            ...
                            logger.info(
                                f"[ServiceManager] Updated file_info: {message['file_info']}"
                            )
                            ...
                            if additional_images:
                                logger.info(
                                    f"[ServiceManager] Downloading {len(additional_images)} additional images from rich text"
                                )
                                downloaded_images = []
                                for idx, add_download_code in enumerate(
                                    additional_images
                                ):
                                    ...
                                    if add_download_result.get("success"):
                                        downloaded_images.append(
                                            {
                                                "name": add_file_name,
                                                "size": add_download_result.get(
                                                    "file_size", 0
                                                ),
                                                "mime_type": "image/jpeg",
                                                "local_path": add_download_result.get(
                                                    "file_path"
                                                ),
                                            }
                                        )
                                if downloaded_images:
                                    message["file_info"]["additional_files"] = (
                                        downloaded_images
                                    )
                                    logger.info(
                                        f"[ServiceManager] Downloaded {len(downloaded_images)} additional images"
                                    )
                                ...
                                try:
                                    ...
                                    if file_content:
                                        ...
                                        if isinstance(original_content, dict):
                                            original_text = original_content.get(
                                                "text", ""
                                            )
                                        else:
                            ...
                            error = download_result.get("error", "Unknown error")
                            logger.error(
                                f"[ServiceManager] Failed to download DingTalk file: {error}"
                            )
                        ...
                        logger.error(
                            f"[ServiceManager] Exception during file download: {download_error}",
                            exc_info=True,
                        )
                        message["file_info"] = {
                            "name": file_name,
                ...
                logger.error(
                    f"[ServiceManager] Error in DingTalk message handler: {e}",
                    exc_info=True,
                )
        ...
        try:
            ...
            with self._lock:
                if key in self._connections:
                    ...
                    self._connections[
                        key
                    ].provider = provider  # 保存 provider 实例供后续使用
            ...
            try:
                ...
                if saved_provider:
                    ...
                    logger.info(
                        f"[ServiceManager] WeChat Work client stopped for {key}"
                    )
            except Exception as e:
                logger.warning(
                    f"[ServiceManager] Error stopping WeChat Work client: {e}"
                )
            raise
        except Exception as e:
    ...
    async def _run_wechat_personal_channel(
        self, sage_user_id: str, config: Dict[str, Any]
    ):
        ...
        try:
            ...
            logger.info(
                f"[ServiceManager] WeChat Personal config: bot_id={bot_id}, bot_token={'***' if bot_token else 'MISSING'}"
            )
            ...
            logger.info(
                f"[ServiceManager] WeChat Personal channel {key} started polling"
            )
    ...
    def _make_message_handler(self, sage_user_id: str, provider_type: str):
        ...
        async def handler(message: Dict[str, Any]):
            ...
            try:
                logger.info("[ServiceManager] ====== Message handler START ======")
                logger.info(
                    f"[ServiceManager] Message from {sage_user_id}:{provider_type}: {message}"
                )
                ...
                if msg_type == "event":
                    logger.info(
                        f"[ServiceManager] Ignoring event type message: {message.get('event_type')}"
                    )
                    return
                ...
                if file_info:
                    logger.info(
                        f"[ServiceManager] File message detected: {file_info.get('name')}"
                    )
                ...
                if not text:
                    logger.warning(
                        f"[ServiceManager] Empty message from {provider_type}"
                    )
                    return
                ...
                async with self._buffer_lock:
                    if buffer_key in self._message_buffers:
                        ...
                        if old_timer:
                            old_timer.cancel()
                        self._message_buffers[buffer_key]["timer"] = (
                            asyncio.create_task(
                        ...
                        logger.info(
                            f"[ServiceManager] Message added to buffer: {buffer_key}, messages={len(self._message_buffers[buffer_key]['messages'])}"
                        )
                    else:
                        # Create new buffer
                        ...
                        logger.info(
                            f"[ServiceManager] New message buffer created: {buffer_key}"
                        )

                logger.info(
                    "[ServiceManager] ====== Message handler END (buffered) ======"
                )

            except Exception as e:
                logger.error(
                    f"[ServiceManager] Error handling message: {e}", exc_info=True
                )
    ...
    async def _delayed_process(
        ...
        await self._process_merged_messages(
            buffer_key, sage_user_id, provider_type, user_id, chat_id, user_name
        )

    async def _process_merged_messages(
        self,
        buffer_key: str,
        sage_user_id: str,
        provider_type: str,
        ...
        logger.info(
            f"[ServiceManager] Processing merged message from {user_id}: {merged_text[:100]}..."
        )
    ...
    async def _handle_slash_command(
        ...
        if cmd in commands:
            try:
                ...
                await self._send_im_response(
                    provider_type, sage_user_id, user_id, chat_id, response
                )
                return True
            except Exception as e:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    data = response.json()
                    if (data.get("success") or data.get("code") == 200) and data.get(
                        "data"
                    ):
                        agents = data["data"]
                        if len(agents) > 0:
                            return agents[0].get("id") or agents[0].get(
                                "agent_id", "default"
                            )
    ...
    async def _send_response_back(
        ...
        try:
            ...
            if not provider:
                ...
                logger.warning(
                    f"[ServiceManager] No running provider for agent={agent_id}, provider={provider_type}, creating new instance"
                )
                ...
                if not provider_config:
                    logger.error(
                        f"[ServiceManager] No config found for agent={agent_id}, provider={provider_type}"
                    )
                    return
                ...
                logger.info(
                    f"[ServiceManager] Reusing existing provider for agent={agent_id}, provider={provider_type}"
                )

            # Send message
            result = await provider.send_message(
                content=content, user_id=user_id, chat_id=chat_id, msg_type="text"
            )
            ...
            if result.get("success"):
                ...
                logger.error(
                    f"[ServiceManager] Failed to send response: {result.get('error')}"
                )

        except Exception as e:
            logger.error(
                f"[ServiceManager] Error sending response back: {e}", exc_info=True
            )

    def find_provider_by_agent(
        self, agent_id: str, provider_type: str
    ) -> Optional[Any]:
        """
        ...
        with self._lock:
            ...
            for key, state in self._connections.items():
                if key == target_key or (
                    ...
                    logger.debug(
                        f"[ServiceManager] Found provider for agent={agent_id}, provider={provider_type}"
                    )
                    return state.provider
