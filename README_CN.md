...
Sage 面向的不是“在 LLM 外面再包一层聊天界面”，而是需要把智能体能力真正做成产品的团队。它把可复用的多智能体运行时与应用层入口放在同一个仓库里，帮助团队从实验走向可用的产品形态。
...
- 支持 `simple`、`multi`、`fibre` 三种多智能体编排与执行模式
- 围绕 Tool、Skill 和 MCP Server 构建的产品级扩展模型
- 内置沙箱机制，提升智能体运行时的安全性
- 支持流式对话、会话工作流和可恢复执行链路
- 在统一运行时基础之上提供 CLI、Web 与桌面端产品形态
- 提供可观测性能力，便于理解、排查和运营复杂智能体行为
