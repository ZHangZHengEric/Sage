...
执行 `deploy/compose.sh <env> up -d` 时，脚本会先复用已经运行的 shared 服务；如果其它环境已经启动了 `sage-rustfs` 等 shared 服务，当前环境只会启动自己的 server/web/mysql，并把 server 接入已存在的 shared 网络。没有任何 shared 服务运行时，脚本会先用 `sage_shared` compose project 启动 shared 服务，再启动当前环境服务。可通过 `SAGE_SHARED_PROJECT_NAME` / `SAGE_SHARED_CONTAINER_PREFIX` 覆盖 shared project 和容器名前缀。
...
```bash
SAGE_REPO_ROOT=$PWD SAGE_DEPLOY_DIR=$PWD/deploy SAGE_CONTAINER_PREFIX=sage-shared docker compose --env-file deploy/dev/.env -f deploy/docker-compose.shared.yml -p sage_shared up -d
...
SAGE_DEPLOY_DIR=$PWD/deploy SAGE_SHARED_NETWORK=sage_shared_default SAGE_CONTAINER_PREFIX=sage-shared docker compose --env-file deploy/dev/.env -p sage_shared -f deploy/docker-compose.observability.yml up -d
```
