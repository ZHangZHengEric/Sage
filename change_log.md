...
- **2026-09-24** 移除 sagents/v2 与 server_v2 的额外 Python 版本运行时校验，删去 compat 模块和对应测试；将应用装配与启动入口收进 main.py，宿主与请求上下文合并到 bootstrap.py；服务类名去掉 V2，`create_app` 只接收 Host；不再在端口占用时切换到随机端口，并合并环境变量读取逻辑。

- **2026-09-23 13:45** server_v2 删除旧目录副本；用例改为 Session 读取与执行端口；对话和包任务共用循环装配。

- **2026-09-23 13:30** server_v2 共用 Run 受理与工具装配；catalog 按资源分列写入；包路由只注入包用例。

- **2026-09-23 12:40** Linux 沙箱未配 cgroup 时仍隔离；CI 不再放宽用户命名空间，需要进沙箱执行的用例在 CI 跳过。
...
- **2026-09-10 16:36** file_update 以内容是否变化为准：删行会写盘，真没变才报 unchanged。

- **2026-09-10 16:35** 去掉 file_write 的 1000 字文案限制，与 v2 一样允许整文件写入。

- **2026-09-10 16:32** YAML 校验拒绝重复 key，与 compose / manifest 严格解析对齐。

- **2026-09-10 16:30** v2 file_update 多个 line_range 改为按原文件行号从下往上应用，与 v1 一致。

- **2026-09-10 16:24** file_read / file_update 行号统一为 1 起步闭区间，与展示行号对齐。
...
- **2026-09-01 19:46** 同能力一文件一插件并声明 plugin_id；登记只留 official.py；官方工具实现迁到 tool/official/；新增 sage.tool.ephemeral；手册只写插件 id。
...
- **2026-08-27 16:00** 启动数据库结构对齐仅依据 ORM 元数据通用增删列和补索引，移除会话列、旧索引及默认 Agent 的定向特判。

- **2026-08-26 19:37** 回滚 tool 结果出参改对象，流式/历史仍返回 JSON 字符串。
